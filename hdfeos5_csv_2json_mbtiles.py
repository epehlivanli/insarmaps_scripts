...
def parse_csv_timeseries(csv_path):
    df = pd.read_csv(csv_path)
    metadata_cols = ['id', 'Latitude', 'Longitude', 'ACC', 'SEASON_AMP', 'VEL_V', 'V_STDEV_V']
    
    date_columns = [col for col in df.columns if col not in metadata_cols]
    date_columns = sorted(date_columns)

    displacements = df[date_columns].to_numpy()  # shape: (N, T)
    decimal_dates = [dec_date(d) for d in date_columns]

    lats = df['Latitude'].to_numpy().reshape(-1, 1)  # (N, 1)
    lons = df['Longitude'].to_numpy().reshape(-1, 1)  # (N, 1)

    # Dict of 2D arrays: {date_str: (N, 1)}
    timeseries_datasets = {
        date: displacements[:, i].reshape(-1, 1)
        for i, date in enumerate(date_columns)
    }

    return decimal_dates, timeseries_datasets, date_columns, lats, lons
# csv ingestion 
...
def create_json(decimal_dates, timeseries_datasets, dates, json_path, folder_name, work_idxs, num_columns, num_rows, lats=None, lons=None, chunk_num=None):
#    global chunk_num
...
def build_parser():
    ...
    parser.add_argument("--csv", help="path to csv file", required=False)
    ...
    parser.add_argument("file", nargs="?", help="Unavco HDFEOS5 file to ingest (not needed with --csv)")
...
def main():
    ...
    if parseArgs.csv:
        ...
        df = pd.read_csv(parseArgs.csv)
        metadata_cols = ['id', 'Latitude', 'Longitude', 'ACC', 'SEASON_AMP', 'VEL_V', 'V_STDEV_V']
        date_columns = sorted([col for col in df.columns if col not in metadata_cols])
        decimal_dates = [dec_date(d) for d in date_columns]

        displacements = df[date_columns].to_numpy()
        lats = df['Latitude'].to_numpy().reshape(-1, 1)
        lons = df['Longitude'].to_numpy().reshape(-1, 1)
        ...
        folder_name = os.path.splitext(os.path.basename(parseArgs.csv))[0]

        # convert_data() manually for csv
        from multiprocessing import Process, Value
        import numpy as np
        global chunk_num
#        chunk_num = Value('i', 0)

        num_points = displacements.shape[0]
        chunk_size = 10000
        ...
        for i in range(num_chunks):
            ...
            end_idx = min((i + 1) * chunk_size - 1, num_points - 1)

            p = Process(target=create_json, args=(
                decimal_dates,
                timeseries_datasets,
                date_columns,
                parseArgs.outputDir,
        ...
        # tippecanoe
        os.chdir(os.path.abspath(parseArgs.outputDir))
        ...
        mid_lat = float(lats[len(lats)//2][0])
        mid_long = float(lons[len(lons)//2][0])
        ...
        string_dates_sql = '{' + ','.join(date_columns) + '}'
        decimal_dates_sql = '{' + ','.join([str(d) for d in decimal_dates]) + '}'

        attribute_keys = '{ACC,SEASON_AMP,VEL_V,V_STDEV_V}'
        attribute_values = '{' + ','.join([
            str(df["ACC"].mean()),
            str(df["SEASON_AMP"].mean()),
            str(df["VEL_V"].mean()),
            str(df["V_STDEV_V"].mean())
        ]) + '}'
        ...
        insarmapsMetadata = {
            ...
            "string_dates_sql": string_dates_sql,
            "decimal_dates_sql": decimal_dates_sql,
            "attributes": {},
            "needed_attributes": {},
        }

        os.makedirs(parseArgs.outputDir, exist_ok=True)
        metadataFilePath = os.path.join(parseArgs.outputDir, "metadata.pickle")
        with open(metadataFilePath, "wb") as f:
            dump(insarmapsMetadata, f)
